    ...
    (() => {
        ...
        function copyTextToClipboard() {
            document.addEventListener("click", (e => {
                ...
                if (copyElement) {
                    const textToCopy = copyElement.querySelector("[data-clipboardEl]").textContent;
                    const currentTippyId = copyElement.getAttribute("aria-describedby");
                    const currentTippyElement = modules_flsModules.tippy.find((item => item.popper.id === currentTippyId));
                    async function copyText(textToCopy) {
                        try {
                            await navigator.clipboard.writeText(textToCopy);
                        } catch (error) {
                            currentTippyElement.setContent("Doesn't copied");
                        } finally {
                            setTimeout((() => {
                                currentTippyElement.hide();
                            }), 1e3);
                        }
                    }
                    copyText(textToCopy);
        ...
        function splitElementIntoSpans(selector, delayBetween, generalDelay) {
            ...
            if (elementsArr.length) elementsArr.forEach((element => {
                ...
                const spannedText = charsArr.map((char => {
                    ...
                    if (char === " ") span.textContent = " "; else span.textContent = char;
        ...
        function createKeyframesAnimation({name, keyframeBody}) {
            let styleSheet = null;
            for (let i = 0; i < document.styleSheets.length; i++) if (document.styleSheets[i].href === null) {
            ...
            for (let i = 0; i < styleSheet.cssRules.length; i++) if (styleSheet.cssRules[i].type === CSSRule.KEYFRAMES_RULE && styleSheet.cssRules[i].name === name) {
        ...
        pageNavigation();
        headerScroll();
        createKeyframesAnimation(mochaTitle);
